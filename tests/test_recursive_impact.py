...
from tessera.db.models import Base, TeamDB, AssetDB, ContractDB, RegistrationDB, AssetDependencyDB
from tessera.main import app
from tessera.models.enums import APIKeyScope, ContractStatus, RegistrationStatus, DependencyType
...
class TestRecursiveImpact:
    ...
    async def test_recursive_impact_depth_3(self, session: AsyncSession, client: AsyncClient):
        ...
        dep_ba = AssetDependencyDB(dependent_asset_id=asset_b.id, dependency_asset_id=asset_a.id, dependency_type=DependencyType.TRANSFORMS)
        dep_cb = AssetDependencyDB(dependent_asset_id=asset_c.id, dependency_asset_id=asset_b.id, dependency_type=DependencyType.TRANSFORMS)
        session.add_all([dep_ba, dep_cb])

        # Active contract for A
        contract_a = ContractDB(asset_id=asset_a.id, version="1.0.0", schema_def={"type": "object", "properties": {"id": {"type": "integer"}}}, published_by=team.id)
        # Active contract for C
        contract_c = ContractDB(asset_id=asset_c.id, version="1.0.0", schema_def={"type": "object"}, published_by=team.id)
        session.add_all([contract_a, contract_c])
        await session.flush()

        # Registration for C
        reg_c = RegistrationDB(contract_id=contract_c.id, consumer_team_id=consumer_team.id, status=RegistrationStatus.ACTIVE)
        session.add(reg_c)
        await session.flush()

        # Analyze impact of breaking change on A
        proposed_schema = {"type": "object", "properties": {"id": {"type": "string"}}} # Breaking change
    ...
    async def test_circular_dependency_safety(self, session: AsyncSession, client: AsyncClient):
        ...
        dep_ab = AssetDependencyDB(dependent_asset_id=asset_b.id, dependency_asset_id=asset_a.id, dependency_type=DependencyType.TRANSFORMS)
        dep_ba = AssetDependencyDB(dependent_asset_id=asset_a.id, dependency_asset_id=asset_b.id, dependency_type=DependencyType.TRANSFORMS)
        session.add_all([dep_ab, dep_ba])

        contract_a = ContractDB(asset_id=asset_a.id, version="1.0.0", schema_def={"type": "object"}, published_by=team.id)
